...
class list_mark:
    ...
    def markList(self, marks, num_students, student_names):
        if not marks or len(marks) == 0:
            self.stdscr.clear()
            self.stdscr.addstr(0, 0, "No marks data available. Please enter marks first.")
            self.stdscr.addstr(1, 0, "Press any key to go back...")
            self.stdscr.getch()
            return
        
        self.stdscr.clear()
        self.stdscr.addstr(0, 0, "=== LIST OF STUDENTS'S MARKS ===\n\n")
        
        line = 2
        for student in student_names:
            self.stdscr.addstr(line, 0, f"Student: {student}")
            line += 1

            student_marks = [m for m in marks if m["Student's name"] == student]
            
            if student_marks:
                for mark in student_marks:
                    self.stdscr.addstr(line, 2, f"- {mark['Course']}: {mark['Mark']}")
                    line += 1
            else:
                self.stdscr.addstr(line, 2, "No marks recorded")
                line += 1
            
            line += 1 
        
        self.stdscr.addstr(line, 0, "Press any key to go back...")
        self.stdscr.getch()