...
def load_from_dat(m, stdscr):
    if os.path.exists("students.dat"):
        ...
        m.set_num_students(data["num_students"])
        m.set_student_ids(data["student_ids"])
        m.set_student_names(data["student_names"])
        m.set_student_births(data["student_births"])
        m.set_num_courses(data["num_courses"])
        m.set_course_ids(data["course_ids"])
        m.set_course_names(data["course_names"])
        m.set_course_credits(data["course_credits"])
        m.set_marks(data["marks"])
        
        stdscr.addstr(3, 0, f"Loaded {data['num_students']} students")
        stdscr.addstr(4, 0, f"Loaded {data['num_courses']} courses")
        stdscr.addstr(5, 0, f"Loaded {len(data['marks'])} marks")
...
def save_to_dat(m):
    data = {
        "num_students": m.get_num_students(),
        "student_ids": m.get_student_ids(),
        "student_names": m.get_student_names(),
        "student_births": m.get_student_births(),
        "num_courses": m.get_num_courses(),
        "course_ids": m.get_course_ids(),
        "course_names": m.get_course_names(),
        "course_credits": m.get_course_credits(),
        "marks": m.get_marks()
    }
